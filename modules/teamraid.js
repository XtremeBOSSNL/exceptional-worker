...
class Raid {
    ...
    constructor(atk, def) {
        ...
        this.def = def.map(x => new Farm(x.hp, new Worker(x.type, x.level)));
...
class RaidCalulator {
    calculateRaidProgress(order, defenders) {
        let raid = new Raid(order, defenders);
        let score = raid.simulate();
        return score;
    }
}

module.exports = {RaidCalulator};